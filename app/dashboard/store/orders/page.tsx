...
import { ORDER_STATUS, PAYMENT_STATUS } from '@/lib/member-constants';
...
export default function AdminOrdersPage() {
  ...
  const handlePayment = async (id: string, paymentStatus: string) => {
    ...
    if (error) toast.error('변경 실패'); else { toast.success('결제 상태가 변경되었습니다.'); refetch(); }
  };
  ...
  return (
    <div className="space-y-5" style={{ maxWidth: '1200px' }}>
      ...
      {loading ? <p className="text-center text-sm text-text-secondary py-10">불러오는 중...</p> : filtered.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(o => {
                ...
                const pStatus = PAYMENT_STATUS?.[o.payment_status as keyof typeof PAYMENT_STATUS];
                return (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        ...
                        {(o.status === 'pending' || o.status === 'processing') && (
                          <button onClick={() => handleStatus(o.id, 'cancelled')} className="px-2 py-1 text-[11px] rounded-md bg-red-light text-red">취소</button>
                        )}
                      </div>
                    </td>
                  </tr>
