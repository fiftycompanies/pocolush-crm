...
export default function MyCouponsPage() {
  ...
  const usedExpiredCoupons = coupons.filter(c => c.status !== 'issued');
        ...
        <>
          ...
          {activeCoupons.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-text-secondary">사용 가능 ({activeCoupons.length})</p>
              {activeCoupons.map(c => {
          ...
          {usedExpiredCoupons.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-text-tertiary mt-4">사용완료 / 만료 ({usedExpiredCoupons.length})</p>
              {usedExpiredCoupons.map(c => {
                const status = COUPON_STATUS[c.status];
                return (
                  <div key={c.id} className="bg-white border border-border rounded-2xl p-4 opacity-50">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-text-primary line-through">{c.coupon?.name}</p>
                      <span className="text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ color: status?.color, backgroundColor: status?.bg }}>
                        {status?.label}
                      </span>
                    </div>
                    <p className="text-[11px] text-text-tertiary mt-1">{c.coupon?.description}</p>
                  </div>
                );
              })}
            </div>
